...
static double MaxBytesForMemLevel(const Options* options, int level) {
  //double result = 5 * 4 * 1024 * 1024.0;  //m1层只有5个表
   ...
   double result =  2 * options->max_table_size;//m0 
   if(level > 0){
     result = 4 * options->max_table_size;
     while(level > 1){
      result *= 5;
      level--;
     }
...
class MemVersion::LevelTableNumIterator : public Iterator {
 public:
  ...
  Slice value() const override {  // pointer
    ...
    //_______
    //这里进入没有问题
    //std::fprintf(stdout,"find mem iter smallest:%s,largest:%s\n",(*flist_)[index_]->mem_table->GetSmallestInternal().data(),(*flist_)[index_]->mem_table->GetLargestInternal().data());
  ...
  for (int level = 1; level < config::kNumMemLevels; level++) {
    ...
    if (index < num_tables) {
      ...
      if (ucmp->Compare(user_key, f->smallest.user_key()) < 0) { 
        ...
        //_____
        /*
        std::fprintf(stdout,"________\n");
        std::fprintf(stdout,"smallest:%s,largest:%s\n",f->smallest.user_key().data(),f->largest.user_key().data());
        */
...
void MemVersionSet::AppendVersion(MemVersion* v) {
  ...
  for (int i=0; i < v->tables_[0].size(); i++)
  { 
    std::fprintf(stdout,"has level 0 mem:%ld,smallest:%s,largest:%s\n",v->tables_[0][i]->number,v->tables_[0][i]->smallest.user_key().data(),v->tables_[0][i]->largest.user_key().data());
  }
  for (int i=0; i < v->tables_[1].size(); i++)
  { 
    std::fprintf(stdout,"has level 1 mem:%ld,smallest:%s,largest:%s\n",v->tables_[1][i]->number,v->tables_[1][i]->smallest.user_key().data(),v->tables_[1][i]->largest.user_key().data());
  }
  for (int i=0; i < v->tables_[2].size(); i++)
  { 
    std::fprintf(stdout,"has level 2 mem:%ld,smallest:%s,largest:%s\n",v->tables_[2][i]->number,v->tables_[2][i]->smallest.user_key().dmouxiangata(),v->tables_[2][i]->largest.user_key().data());
  }
  */
...
void MemVersionSet::Finalize(MemVersion* v) {
  ...
  //达到write_buffer_size，flush,会出现内存
...
MemTableMetaData* MemVersionSet::PickFlush() {//轮讯选择一个sst
  ...
  for(int i = 0; i <= config::kNumMemLevels-1; i++){
    if(current_->tables_[i].size() > 0) level = std::max(level ,i);
  }
  ...
  // Pick the first file that comes after flush_pointer_
   ...
   // std:fprintf(stdout,"disk flush，level：%d \n",level);
  return meta;
}
*/
MemTableMetaData* MemVersionSet::PickFlush() {//选择最底层第一个memtable
  int level = 0;
  for(int i = 0; i < config::kNumMemLevels-1; i++){
    if(current_->tables_[i].size() > 0) level = std::max(level ,i);
  }
  MemTableMetaData* meta = current_->tables_[level][0];
  return meta;//选择最底层第一个memtable
}
